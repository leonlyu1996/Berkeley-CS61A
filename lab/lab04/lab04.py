...
def closer_city(lat, lon, city1, city2):
    ...
    if distance(temp_city, city1) < distance(temp_city, city2):
...
def put_piece(board, max_rows, column, player):
    ...
    "*** YOUR CODE HERE ***"
    row = max_rows - 1
    while get_piece(board, row, column) != '-':
        if row == 0:
            return -1, board
        row = row - 1
    new_lst = replace_elem(board[row], column, player)
    new_board = replace_elem(board, row, new_lst)
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        ...
        for col in range(max_cols):
            print_row += get_piece(board, row, col) + ' '
        print(print_row.strip())
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for col in range(max_cols):
        if get_piece(board, row, col) == player:
            ...
            adjacent_number = 0
        if adjacent_number == num_connect:
            return True
    return False
