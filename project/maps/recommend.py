...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        ...
        closest_list = [find_closest(location, centroids), restaurant]
        cluster.append(closest_list)
...
def find_predictor(user, restaurants, feature_fn):
    ...
    s_xx = sum([square(x - mean(xs)) for x in xs])
    s_yy = sum([square(y - mean(ys)) for y in ys])
    s_xy = sum([(xs[i] - mean(xs)) * (ys[i] - mean(ys)) for i in range(len(xs))])

    b = s_xy / s_xx
    a = mean(ys) - b * mean(xs)
    r_squared = square(s_xy) / (s_xx * s_yy) 
    # END Question 7
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            all_rate[name] = user_rating(user, name)
        else:
            all_rate[name] = predictor(restaurant)
    return all_rate
    # END Question 9
