...
def balanced(m):
    ...
    if length(left_side) * total_weight(end(left_side)) != length(right_side) * total_weight(end(right_side)):
        return False
    return not False in [balanced(end(side)) for side in sides(m) if not is_weight(end(side))]
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_password):
        ...
        if input_password != password:
            incorrect_password_list.append(input_password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    test_result = withdraw(0, old_password)
